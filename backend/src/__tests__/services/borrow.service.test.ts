...
function mockPrisma(overrides: Record<string, any> = {}) {
  const defaults = {
    ...
    borrowRecord: { findFirst: vi.fn(), findUnique: vi.fn(), findMany: vi.fn(), create: vi.fn(), update: vi.fn(), count: vi.fn() },
...
beforeEach(() => {
  ...
  vi.mocked(rules.getRule).mockResolvedValue({
    maxBorrows: 5, loanDays: 30, renewals: 1, renewalDays: 15, finePerDay: 0.10
  })
  vi.mocked(rules.checkBorrowLimit).mockResolvedValue({ allowed: true, currentCount: 2, maxBorrows: 5 })
  vi.mocked(fines.calcOverdueFine).mockReturnValue(5.00)
  vi.mocked(fines.createFine).mockResolvedValue({ amount: 5.00, type: 'overdue' })
})

describe('borrow', () => {
  it('1. 正常借书 — 创建记录 + 库存-1 + 复本状态变 borrowed', async () => {
    const prisma = mockPrisma()
    prisma.bookItem.findUnique = vi.fn().mockResolvedValue({ id: 1, bookId: 10, status: 'available', itemTypeId: 1, book: { id: 10 } })
    ...
    prisma.borrowRecord.create = vi.fn().mockResolvedValue({ id: 100, bookId: 10, bookItemId: 1, status: 'active',
  ...
  it('2. 读者已有此书 — 返回错误 "already borrowed"', async () => {
    ...
    await expect(borrowService.borrow(prisma, 1, { bookId: 10, bookItemId: 1 }))
      .rejects.toThrow('already borrowed')
  })

  it('3. 超借阅上限 — 返回错误 "exceeded limit"', async () => {
    vi.mocked(rules.checkBorrowLimit).mockResolvedValue({ allowed: false, currentCount: 5, maxBorrows: 5, message: '已达到借阅上限 5 册' })
    const prisma = mockPrisma()
    prisma.bookItem.findUnique = vi.fn().mockResolvedValue({ id: 1, bookId: 10, status: 'available', itemTypeId: 1, book: { id: 10 } })
    prisma.borrowRecord.findFirst = vi.fn().mockResolvedValue(null)
    prisma.user.findUnique = vi.fn().mockResolvedValue({ id: 1, patronCategoryId: 1 })
    ...
    await expect(borrowService.borrow(prisma, 1, { bookId: 10, bookItemId: 1 }))
      .rejects.toThrow('已达到借阅上限')
  })

  it('4. 无可用复本 — 返回错误 "not available"', async () => {
    const prisma = mockPrisma()
    prisma.bookItem.findUnique = vi.fn().mockResolvedValue({ id: 1, bookId: 10, status: 'borrowed', itemTypeId: 1, book: { id: 10 } })

    await expect(borrowService.borrow(prisma, 1, { bookItemId: 1 }))
...
describe('returnBook', () => {
  it('5. 正常还书 — 记录更新 + 库存+1 + 复本变 available', async () => {
    const record = {
      id: 100, bookId: 10, bookItemId: 1, userId: 1, status: 'active',
    ...
    const prisma = mockPrisma({
      borrowRecord: { findUnique: vi.fn().mockResolvedValue(record), update: vi.fn().mockResolvedValue({ id: 100, status: 'returned' }) },
    })
  ...
  it('6. 逾期还书 — 自动创建 Fine + 计算罚款', async () => {
    const record = {
      id: 100, bookId: 10, bookItemId: 1, userId: 2, status: 'active',
    ...
    const prisma = mockPrisma({
      borrowRecord: { findUnique: vi.fn().mockResolvedValue(record), update: vi.fn().mockResolvedValue({ id: 100, status: 'overdue' }) },
    })
    const result = await borrowService.returnBook(prisma, 100, 2, false)
    expect(result.status).toBe('overdue')
    expect(result.fine).toEqual({ amount: 5.00, type: 'overdue' })
  ...
  it('7. 已还书不能再次还 — 返回错误 "Already returned"', async () => {
    const prisma = mockPrisma({
      borrowRecord: { findUnique: vi.fn().mockResolvedValue({ id: 100, status: 'returned', userId: 1 }) },
    })
    await expect(borrowService.returnBook(prisma, 100, 1, false))
...
describe('renew', () => {
  it('8. 正常续借 — 到期日延长 + renewed=true', async () => {
    ...
    const record = {
      id: 100, userId: 1, status: 'active', renewed: false,
      dueDate, bookItem: { itemTypeId: 1 }, user: { patronCategoryId: 1 },
    }
    const newDue = new Date(dueDate.getTime() + 15 * 86400000)
    const prisma = mockPrisma({
      borrowRecord: { findUnique: vi.fn().mockResolvedValue(record), update: vi.fn().mockResolvedValue({ id: 100, dueDate: newDue, renewed: true }) },
    })
  ...
  it('9. 已续借过 — 返回错误 "Already renewed"', async () => {
    const prisma = mockPrisma({
      borrowRecord: { findUnique: vi.fn().mockResolvedValue({ id: 100, userId: 1, status: 'active', renewed: true, bookItem: { itemTypeId: 1 }, user: { patronCategoryId: 1 } }) },
    })
    await expect(borrowService.renew(prisma, 100, 1))
